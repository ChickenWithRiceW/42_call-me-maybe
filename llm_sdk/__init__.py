...
logging.set_verbosity_error()  # Sets verbosity to only show errors.
...
class Small_LLM_Model:
    ...
    def encode(self, text: str) -> torch.Tensor:
        """Tokenize *text* and return a 2D ``input_ids`` tensor on the target device.
        Ids are wrapped in the first index of the Tensor object, you will need to convert it
        to a list if you want to use it for `get_logits_from_input_ids`

        Example:
            get_logits_from_input_ids(encoded_value[0].tolist())"""
    ...
    def decode(self, ids: torch.Tensor | list[int]) -> str:
        ...
        Input ether the `Tensor` object or a `list of integer`

        If input is `Tensor` object it will be converted automatically to list
        
        Return:
            A normal string decoded from the tokens"""
    ...
    def get_path_to_tokenizer_file(self) -> str:
        """Returns a path to a json file called 'tokenizer.json' that hol"""
